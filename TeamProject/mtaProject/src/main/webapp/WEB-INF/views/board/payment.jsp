    ...
    <table class="table table-hover">
            ...
            <tr>
                <td><label for="user_name"> Name : 
                    <input type="text" name="user_name" id="user_name" readonly ="readonly"/>
                </label></td>
            </tr>
            <tr>
                <td>
			...
			<table summary="장바구니 리스트" class="table">
				...
				<tbody id = "list" class ="table table-hover">
					...
					<c:choose>
						...
						<td class = "text-left"><input type ="checkbox" name = "allCheck" id = "allCheck" class ="" checked="checked"/><label for ="allCheck" class ="">&nbsp;&nbsp;&nbsp;전체선택</label></td></tr>
						<c:set var="sum" value="0" />
							<c:forEach var ="cart" items="${cartList}" varStatus="status"> <!-- items 의 항목을 모두 반복 -->
								<tr class ="text-center" > 
									<td class ="text-left"><input type="checkbox" class ="chkbox" name ="check" disabled="disabled"  data-cartNum ="${cart.cart_id}" data-mPrice= "${cart.m_price}" value ="${cart.m_price}"/></td>
						...
						<c:otherwise> <!-- if문의 else -->
							<tr>
								<td colspan="4" class ="tac text-center">결제 할 상품이 없습니다.</td>
							</tr>
						</c:otherwise>
					</c:choose>
				</tbody>
